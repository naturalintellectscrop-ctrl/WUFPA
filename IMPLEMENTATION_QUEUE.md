...
>
> **31 July 2026, later the same day — that outstanding pass immediately found a real bug,
> which is exactly why the box was left unticked instead of assumed.** The client opened the
> live dev server on a narrow viewport, opened the mobile menu, and could not close it — the
> open panel is `position: fixed; inset: 0` at `--z-modal` (400), which paints over the sticky
> header at `--z-sticky` (100) the only close control lived in. The trigger was still in the
> DOM and still reachable by keyboard (so the build-time structural checks never caught it),
> just not visible or tappable once the panel it controlled covered it. **Fixed:** a dedicated
> close button now lives inside the panel itself (`data-mobile-close`), revealed by
> `mobile-nav.ts` alongside the trigger; the header's own trigger hides itself while the panel
> is open (redundant and covered anyway) and reappears on close. Focus now moves to the new
> close button on open, not the first link, so a keyboard user's first action inside the dialog
> is always "how do I leave" before "where do I go." This is the concrete argument for why
> `QC-UI`'s screen-reader/keyboard pass is a real gate and not a formality — every automated
> check in this project's guard chain passed throughout, because none of them render a page in
> an actual browser and try to use it.
